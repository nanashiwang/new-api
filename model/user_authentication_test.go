...
func TestValidateBackupCodeCanOnlySucceedOnceConcurrently(t *testing.T) {
	truncateTables(t)

	const code = "ABCD-1234"
	user := &User{Username: "twofa-backup-concurrent", Password: "password"}
	require.NoError(t, DB.Create(user).Error)
	require.NoError(t, CreateBackupCodes(user.Id, []string{code}))
	...
	for i := 0; i < attempts; i++ {
		...
		go func() {
			defer wg.Done()
			valid, err := ValidateBackupCode(user.Id, code)
			results <- valid
			errs <- err
		}()
	}
