...
func TestComputeTieredQuota_WithTimeRatio(t *testing.T) {
	exprStr := `tier("default", p + c)`
	snap := &billingexpr.BillingSnapshot{
		BillingMode:  "tiered_expr",
		ExprString:   exprStr,
	...
	// exprOutput = 1500; quotaBeforeGroup = 750; after = round(750 * 2.0 * 1.5) = 2250
	if result.ActualQuotaAfterGroup != 2250 {
		t.Errorf("after group = %d, want 2250", result.ActualQuotaAfterGroup)
	}
}
