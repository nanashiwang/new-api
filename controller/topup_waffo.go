...
func WaffoWebhook(c *gin.Context) {
	...
	if err := common.Unmarshal(bodyBytes, &event); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo webhook 解析失败 path=%q client_ip=%s error=%q body_len=%d", c.Request.RequestURI, c.ClientIP(), err.Error(), len(bodyBytes)))
	...
	case core.EventPayment:
		...
		if err := common.Unmarshal(bodyBytes, &payload); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 支付回调载荷解析失败 event_type=%s client_ip=%s error=%q body_len=%d", event.EventType, c.ClientIP(), err.Error(), len(bodyBytes)))
...
func handleWaffoPayment(c *gin.Context, wh *core.WebhookHandler, result *core.PaymentNotificationResult) {
	...
	providerAmount, _ := strconv.ParseFloat(result.FinalDealAmount, 64)
	if providerAmount <= 0 {
		providerAmount, _ = strconv.ParseFloat(result.OrderAmount, 64)
	}
	expectedCurrency := strings.ToUpper(strings.TrimSpace(getWaffoCurrency()))
	if expectedCurrency == "" {
		expectedCurrency = "USD"
	}
	if strings.ToUpper(strings.TrimSpace(result.OrderCurrency)) != expectedCurrency {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo webhook 币种不匹配 trade_no=%s expected=%s got=%s", merchantOrderId, expectedCurrency, result.OrderCurrency))
		sendWaffoWebhookResponse(c, wh, false, "currency mismatch")
		return
	}
	if topUp := model.GetTopUpByTradeNo(merchantOrderId); topUp != nil {
		if webhookUserID := getWaffoNotificationUserID(result); webhookUserID != "" && webhookUserID != strconv.Itoa(topUp.UserId) {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo webhook 用户不匹配 trade_no=%s expected_user_id=%d got_user_id=%s", merchantOrderId, topUp.UserId, webhookUserID))
			sendWaffoWebhookResponse(c, wh, false, "user mismatch")
			return
		}
	}
	validationInput := service.PaymentCallbackValidationInput{
		...
		Source:          "waffo_webhook",
		ProviderPayload: common.GetJsonString(result),
	}
	checkResult, err := service.ValidateTopUpCallback(validationInput)
	if err != nil {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo 充值校验失败 trade_no=%s client_ip=%s error=%q", merchantOrderId, c.ClientIP(), err.Error()))
...
func getWaffoNotificationUserID(result *core.PaymentNotificationResult) string {
	if result == nil || result.UserInfo == nil {
		return ""
	}
	for _, key := range []string{"userId", "userID", "user_id", "UserID"} {
		if value, ok := result.UserInfo[key]; ok {
