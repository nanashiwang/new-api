...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 收到请求 path=%q client_ip=%s body_len=%d", c.Request.RequestURI, c.ClientIP(), len(bodyBytes)))
	...
	providerAmount, err := strconv.ParseFloat(string(event.Data.Amount), 64)
	if err != nil || providerAmount <= 0 {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 金额无效 trade_no=%s event_id=%s amount=%q", tradeNo, event.ID, event.Data.Amount))
		c.String(http.StatusOK, "OK")
		return
	}
	...
	if strings.ToUpper(strings.TrimSpace(event.Data.Currency)) != expectedCurrency {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 币种不匹配 trade_no=%s event_id=%s expected=%s got=%s", tradeNo, event.ID, expectedCurrency, event.Data.Currency))
		c.String(http.StatusOK, "OK")
		return
	}
	...
	checkResult, err := service.ValidateTopUpCallback(service.PaymentCallbackValidationInput{
		...
		Source:          "waffo_pancake_webhook",
		ProviderPayload: common.GetJsonString(event),
	})
	if err != nil {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo Pancake 充值校验失败 trade_no=%s event_id=%s error=%q", tradeNo, event.ID, err.Error()))
		c.String(http.StatusOK, "OK")
		return
	}
	...
	if err := model.RechargeWaffoPancake(tradeNo); err != nil {
		...
		if errors.Is(err, model.ErrPaymentMethodMismatch) || strings.Contains(err.Error(), "not pending") {
			c.String(http.StatusOK, "OK")
			return
		}
