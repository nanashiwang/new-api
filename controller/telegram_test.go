...
func TestVerifyTelegramAuthorization(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "empty token", authDate: now, mutate: nil, wantErr: "token"},
		...
		{name: "missing auth date", authDate: now, mutate: func(values url.Values) { values.Del("auth_date") }, wantErr: "incomplete"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			telegramID, err := verifyTelegramAuthorization(params, verifyToken, now)
			if tt.wantErr != "" {
				require.Error(t, err)
