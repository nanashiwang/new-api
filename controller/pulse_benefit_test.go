...
func TestPulseBenefitRejectsTrailingJSON(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/api/internal/pulse/benefits/grant", bytes.NewBufferString(`{"grant_id":"grant-1","user_id":42,"amount":10,"source_ref":"grant-1","reward_type":"period"}{"unexpected":true}`))
	ctx.Request.Header.Set("Content-Type", "application/json")
	ctx.Set("pulse_service_user_id", uint64(42))
