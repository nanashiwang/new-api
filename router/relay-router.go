...
func SetRelayRouter(router *gin.Engine) {
	router.Use(middleware.RequestFailureLog())
	router.Use(middleware.CORS())
	router.Use(middleware.DecompressRequestMiddleware())
	router.Use(middleware.BodyStorageCleanup()) // 清理请求体存储
	router.Use(middleware.StatsMiddleware())
	router.GET("/v1/bridge/media/:id", controller.RelayResponsesMediaBridge)
	// https://platform.openai.com/docs/api-reference/introduction
	modelsRouter := router.Group("/v1/models")
	modelsRouter.Use(middleware.RouteTag("relay"))
	modelsRouter.Use(middleware.TokenAuth())
	modelsRouter.Use(middleware.TokenRuntimeLimit())
	{
	...
	geminiRouter.Use(middleware.TokenRuntimeLimit())
	{
	...
	geminiCompatibleRouter.Use(middleware.TokenRuntimeLimit())
	{
	...
	relayV1Router.Use(middleware.TokenRuntimeLimit())
	...
	relaySunoRouter.Use(middleware.TokenAuth(), middleware.TokenRuntimeLimit(), middleware.Distribute())
	{
	...
	// countTokens is not implemented: never turn a counting request into a
	// billable generation. Reject before authentication and channel selection.
	...
	relayGeminiRouter.Use(middleware.TokenRuntimeLimit())
...
func registerMjRouterGroup(relayMjRouter *gin.RouterGroup) {
	relayMjRouter.Use(middleware.TokenAuth(), middleware.TokenRuntimeLimit(), middleware.Distribute())
	{
