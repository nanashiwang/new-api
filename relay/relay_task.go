...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	// 4. 价格计算：基础模型价格
	info.OriginModelName = modelName
	_, tieredTaskBilling := billing_setting.GetTieredExpr(modelName)
	var err error
	info.PriceData, err = helper.ModelPriceHelperTask(c, info)
	if err != nil {
		return nil, service.TaskErrorWrapper(err, "model_price_failed", http.StatusBadRequest)
	}

	// 5. 计费估算：让适配器根据用户请求提供 OtherRatios（时长、分辨率等）
	//    必须在 ModelPriceHelperPerCall 之后调用（它会重建 PriceData）。
	//    ResolveOriginTask 可能已在 remix 路径中预设了 OtherRatios，此处合并。
	if !tieredTaskBilling {
	...
	if !tieredTaskBilling && !common.StringsContains(constant.TaskPricePatches, modelName) {
