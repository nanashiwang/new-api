...
function normalizeSuitePlanGroup(value: unknown): SuitePlanGroup {
  return value === 'detail' || value === 'promotion' || value === 'sku' ? value : 'main'
}

function normalizeProductAssetKind(value: unknown): ProductAssetKind {
  return value === 'angle' || value === 'sku' || value === 'brand' || value === 'style' ? value : 'main'
}

function normalizeProductAsset(value: unknown): ProductAsset | null {
  if (!isRecord(value) || typeof value.id !== 'string' || typeof value.imageId !== 'string') return null
  return {
    id: value.id,
    kind: normalizeProductAssetKind(value.kind),
    imageId: value.imageId,
    ...(typeof value.name === 'string' ? { name: value.name } : {}),
    ...(typeof value.skuName === 'string' ? { skuName: value.skuName } : {}),
...
function normalizeEcommerceBrief(value: unknown): EcommerceBrief {
  const defaults = createEcommerceSuite().brief
  if (!isRecord(value)) return defaults
  const counts = isRecord(value.counts) ? value.counts : {}
  return {
    productName: typeof value.productName === 'string' ? value.productName : defaults.productName,
    category: typeof value.category === 'string' ? value.category : defaults.category,
    sellingPoints: normalizeSellingPoints(value.sellingPoints),
    targetAudience: typeof value.targetAudience === 'string' ? value.targetAudience : defaults.targetAudience,
    targetPlatforms: Array.isArray(value.targetPlatforms) ? value.targetPlatforms.filter((item): item is string => typeof item === 'string') : defaults.targetPlatforms,
    stylePresetId: typeof value.stylePresetId === 'string' ? value.stylePresetId : defaults.stylePresetId,
    suiteTemplateId: typeof value.suiteTemplateId === 'string' ? value.suiteTemplateId : defaults.suiteTemplateId,
    sizePreset: typeof value.sizePreset === 'string' ? value.sizePreset : defaults.sizePreset,
    extraPrompt: typeof value.extraPrompt === 'string' ? value.extraPrompt : defaults.extraPrompt,
    negativePrompt: typeof value.negativePrompt === 'string' ? value.negativePrompt : defaults.negativePrompt,
    lockProduct: value.lockProduct !== false,
    lockBrandStyle: value.lockBrandStyle !== false,
    allowBeautify: Boolean(value.allowBeautify),
    strictStructure: Boolean(value.strictStructure),
    counts: {
      main: normalizePlanCount(counts.main, defaults.counts.main),
      detail: normalizePlanCount(counts.detail, defaults.counts.detail),
      promotion: normalizePlanCount(counts.promotion, defaults.counts.promotion),
      sku: normalizePlanCount(counts.sku, defaults.counts.sku),
    },
  }
}

function normalizePlanCount(value: unknown, fallback: number) {
  const numeric = Number(value)
  if (!Number.isFinite(numeric)) return fallback
  return Math.min(12, Math.max(0, Math.trunc(numeric)))
}

function normalizeSuitePlanItem(value: unknown): SuitePlanItem | null {
  if (!isRecord(value) || typeof value.id !== 'string') return null
  const status = value.status === 'generating'
    ? 'error'
    : value.status === 'done' || value.status === 'partial_success' || value.status === 'error'
    ? value.status
    : 'draft'
  return {
    id: value.id,
    group: normalizeSuitePlanGroup(value.group),
    title: typeof value.title === 'string' ? value.title : '未命名图片',
    ...
    finalPromptDraft: typeof value.finalPromptDraft === 'string' ? value.finalPromptDraft : typeof value.prompt === 'string' ? value.prompt : '',
    count: normalizePlanCount(value.count, 1) || 1,
    enabled: value.enabled !== false,
    status,
    outputTaskIds: normalizeStringArray(value.outputTaskIds),
    error: typeof value.error === 'string' ? value.error : null,
  }
}

function normalizeEcommerceSuite(value: unknown): EcommerceSuite | null {
  if (!isRecord(value) || typeof value.id !== 'string') return null
  return {
    id: value.id,
    name: typeof value.name === 'string' && value.name.trim() ? value.name : '电商套图',
    brief: normalizeEcommerceBrief(value.brief),
    assets: Array.isArray(value.assets) ? value.assets.map(normalizeProductAsset).filter((asset): asset is ProductAsset => Boolean(asset)) : [],
    plan: Array.isArray(value.plan) ? value.plan.map(normalizeSuitePlanItem).filter((item): item is SuitePlanItem => Boolean(item)) : [],
    createdAt: typeof value.createdAt === 'number' ? value.createdAt : Date.now(),
    updatedAt: typeof value.updatedAt === 'number' ? value.updatedAt : Date.now(),
  }
}

function normalizeEcommerceSuites(value: unknown): EcommerceSuite[] {
  if (!Array.isArray(value)) return []
...
function normalizeEcommerceCapabilities(value: unknown): EcommerceCapabilities {
  ...
  return {
    defaultImageModel: typeof record.defaultImageModel === 'string' && record.defaultImageModel.trim()
      ? record.defaultImageModel.trim()
      : DEFAULT_ECOMMERCE_CAPABILITIES.defaultImageModel,
    defaultPlanModel: typeof record.defaultPlanModel === 'string' && record.defaultPlanModel.trim()
      ? record.defaultPlanModel.trim()
      : DEFAULT_ECOMMERCE_CAPABILITIES.defaultPlanModel,
    supportsEcommerce,
    disabledReason: typeof record.disabledReason === 'string' ? record.disabledReason : supportsEcommerce ? undefined : '当前分组未同时支持 gpt-image-2 与 gpt-5.5',
  }
}
...
export function getPersistedState(state: AppState) {
  ...
  return {
    ...
    ecommerceSuites: state.ecommerceSuites,
    activeEcommerceSuiteId: state.activeEcommerceSuiteId,
...
function mergePersistedState(persistedState: unknown, currentState: AppState): AppState {
  ...
  const appMode = persisted.appMode === 'agent' || persisted.appMode === 'ecommerce' ? persisted.appMode : 'gallery'
  const ecommerceSuites = normalizeEcommerceSuites(persisted.ecommerceSuites)
  const activeEcommerceSuiteId = typeof persisted.activeEcommerceSuiteId === 'string' && ecommerceSuites.some((suite) => suite.id === persisted.activeEcommerceSuiteId)
    ? persisted.activeEcommerceSuiteId
    : ecommerceSuites[0]?.id ?? null
...
interface AppState {
  ...
  // 电商套图
  ecommerceSuites: EcommerceSuite[]
  activeEcommerceSuiteId: string | null
  selectedEcommercePlanItemId: string | null
  ...
  updateEcommerceBrief: (patch: Partial<EcommerceBrief>) => void
  addEcommerceAsset: (kind: ProductAssetKind, imageId: string, name?: string) => void
  removeEcommerceAsset: (assetId: string) => void
  updateEcommercePlanItem: (itemId: string, patch: Partial<SuitePlanItem>) => void
  setSelectedEcommercePlanItemId: (id: string | null) => void
  generateEcommercePlan: () => Promise<void>
...
function isImageReferencedByState(state: AppState, imageId: string) {
  ...
  if (state.ecommerceSuites.some((suite) => suite.assets.some((asset) => asset.imageId === imageId))) return true
...
function syncActiveInputDraft<T extends Partial<AgentInputDraft>>(
  ...
  if (state.appMode !== 'agent') return patch
...
function ensureActiveEcommerceSuite(state: AppState): { suites: EcommerceSuite[]; suite: EcommerceSuite } {
  const existing = state.ecommerceSuites.find((suite) => suite.id === state.activeEcommerceSuiteId)
  if (existing) return { suites: state.ecommerceSuites, suite: existing }

  const suite = createEcommerceSuite()
  return {
    suites: [suite, ...state.ecommerceSuites],
    suite,
  }
}

function updateActiveEcommerceSuiteState(
  state: AppState,
  updater: (suite: EcommerceSuite) => EcommerceSuite,
): Pick<AppState, 'ecommerceSuites' | 'activeEcommerceSuiteId'> {
  const { suites, suite } = ensureActiveEcommerceSuite(state)
  const updatedSuite = { ...updater(suite), updatedAt: Date.now() }
  return {
    ecommerceSuites: suites.map((item) => item.id === updatedSuite.id ? updatedSuite : item),
    activeEcommerceSuiteId: updatedSuite.id,
  }
}

function updateSuitePlanItem(suite: EcommerceSuite, itemId: string, patch: Partial<SuitePlanItem>): EcommerceSuite {
  return {
    ...suite,
    plan: suite.plan.map((item) => item.id === itemId ? { ...item, ...patch } : item),
  }
}
    ...
    (set, get) => ({
      ...
      setAppMode: (appMode) => {
        ...
        if (appMode === 'ecommerce') {
          if (!state.ecommerceCapabilities.supportsEcommerce) {
            state.setConfirmDialog({
              title: '电商套图暂不可用',
              message: state.ecommerceCapabilities.disabledReason || '当前令牌分组未同时支持 gpt-image-2 与 gpt-5.5，无法启用电商套图工作台。',
              showCancel: false,
              confirmText: '我知道了',
              icon: 'info',
              action: () => {},
          ...
          const galleryInputDraft = saveGalleryInputDraft(state)
          set((latest) => {
            const { suites, suite } = ensureActiveEcommerceSuite(latest)
            return {
              appMode: 'ecommerce',
              ecommerceSuites: suites,
      ...
      // Ecommerce
      ecommerceSuites: [],
      activeEcommerceSuiteId: null,
      selectedEcommercePlanItemId: null,
      ecommercePlanDraftStatus: 'idle',
      ecommerceCapabilities: { ...DEFAULT_ECOMMERCE_CAPABILITIES },
      generationQueue: [],
      setEcommerceCapabilities: (capabilities) => set((state) => ({
        ecommerceCapabilities: normalizeEcommerceCapabilities({ ...state.ecommerceCapabilities, ...capabilities }),
      })),
      createEcommerceSuite: () => {
        const suite = createEcommerceSuite()
        set((state) => ({
          ecommerceSuites: [suite, ...state.ecommerceSuites],
          activeEcommerceSuiteId: suite.id,
          selectedEcommercePlanItemId: null,
          ecommercePlanDraftStatus: 'idle',
        }))
      ...
      setActiveEcommerceSuiteId: (activeEcommerceSuiteId) => set({ activeEcommerceSuiteId, selectedEcommercePlanItemId: null }),
      updateEcommerceBrief: (patch) => set((state) => {
        const partial = updateActiveEcommerceSuiteState(state, (suite) => ({
          ...suite,
          name: patch.productName && patch.productName.trim() ? `${patch.productName.trim()}套图` : suite.name,
          brief: {
            ...suite.brief,
            ...patch,
            counts: patch.counts ? { ...suite.brief.counts, ...patch.counts } : suite.brief.counts,
          },
        }))
        return { ...partial, ecommercePlanDraftStatus: 'idle' }
      }),
      addEcommerceAsset: (kind, imageId, name) => set((state) => {
        const partial = updateActiveEcommerceSuiteState(state, (suite) => {
          const limit = getBucketLimit(kind)
          const sameKind = suite.assets.filter((asset) => asset.kind === kind)
          if (sameKind.some((asset) => asset.imageId === imageId)) return suite
          const withoutOverflow = sameKind.length >= limit
            ? suite.assets.filter((asset) => asset.kind !== kind || asset.id !== sameKind[0]?.id)
            : suite.assets
          return {
            ...suite,
            assets: [...withoutOverflow, createProductAsset(kind, imageId, name)],
          }
        })
        return partial
      }),
      removeEcommerceAsset: (assetId) => {
        let removedImageId: string | null = null
        set((state) => updateActiveEcommerceSuiteState(state, (suite) => {
          const removed = suite.assets.find((asset) => asset.id === assetId)
          removedImageId = removed?.imageId ?? null
          return {
            ...suite,
            assets: suite.assets.filter((asset) => asset.id !== assetId),
          }
        }))
        if (removedImageId) void deleteImageIfUnreferenced(removedImageId)
      },
      updateEcommercePlanItem: (itemId, patch) => set((state) => updateActiveEcommerceSuiteState(state, (suite) => updateSuitePlanItem(suite, itemId, patch))),
      setSelectedEcommercePlanItemId: (selectedEcommercePlanItemId) => set({ selectedEcommercePlanItemId }),
      generateEcommercePlan: async () => {
        const state = useStore.getState()
        const activeProfile = getActiveApiProfile(state.settings)
        const { suites, suite } = ensureActiveEcommerceSuite(state)
        if (!suite.brief.productName.trim()) {
          state.showToast('请先填写商品名称', 'error')
          return
        }
        if (!suite.assets.some((asset) => asset.kind === 'main')) {
          state.showToast('请至少上传一张主商品图', 'error')
          return
        }

        useStore.setState({ ecommerceSuites: suites, activeEcommerceSuiteId: suite.id, ecommercePlanDraftStatus: 'planning' })
        const skeleton = createSuiteSkeleton(suite.brief)
        try {
          const plan = await callEcommercePlanApi({
            settings: state.settings,
            profile: activeProfile,
            capabilities: state.ecommerceCapabilities,
            brief: suite.brief,
            assets: suite.assets,
            skeleton,
          })
          useStore.setState((latest) => updateActiveEcommerceSuiteState(latest, (currentSuite) => currentSuite.id === suite.id ? { ...currentSuite, plan } : currentSuite))
          useStore.setState({ ecommercePlanDraftStatus: 'ready', selectedEcommercePlanItemId: plan[0]?.id ?? null })
          useStore.getState().showToast('套图方案已生成', 'success')
        } catch (err) {
          const plan = fallbackEcommercePlan(suite.brief, suite.assets, skeleton)
          useStore.setState((latest) => updateActiveEcommerceSuiteState(latest, (currentSuite) => currentSuite.id === suite.id ? { ...currentSuite, plan } : currentSuite))
          useStore.setState({ ecommercePlanDraftStatus: 'ready', selectedEcommercePlanItemId: plan[0]?.id ?? null })
          useStore.getState().showToast(`规划模型不可用，已使用规则方案：${err instanceof Error ? err.message : String(err)}`, 'error')
        }
      },
      generateEcommerceSuite: async () => {
        await runEcommerceGeneration()
      },
      retryEcommercePlanItem: async (itemId) => {
        await runEcommerceGeneration(itemId)
      },
      addEcommerceStyleReference: (imageId, name) => set((state) => updateActiveEcommerceSuiteState(state, (suite) => {
        if (suite.assets.some((asset) => asset.kind === 'style' && asset.imageId === imageId)) return suite
        const limit = getBucketLimit('style')
        const styleAssets = suite.assets.filter((asset) => asset.kind === 'style')
        const overflowId = styleAssets.length >= limit ? styleAssets[0]?.id : null
        return {
          ...suite,
          assets: [
            ...suite.assets.filter((asset) => asset.id !== overflowId),
            createProductAsset('style', imageId, name),
          ],
        }
      })),
...
function applyTaskApiProfileOverrides(profile: ApiProfile, task: TaskRecord): ApiProfile {
  if (!task.apiModel && !task.apiMode) return profile
  return {
    ...profile,
    model: task.apiModel || profile.model,
    apiMode: task.apiMode || profile.apiMode,
  }
}
...
export async function submitEcommerceImageTask(options: {
  prompt: string
  inputImageIds: string[]
  params: TaskParams
  suiteId: string
  suiteGroup: SuitePlanGroup
  suiteItemId: string
  suiteItemTitle: string
}): Promise<string> {
  const { settings, ecommerceCapabilities, showToast } = useStore.getState()
  const normalizedSettings = normalizeSettings(settings)
  const activeProfile = getActiveApiProfile(normalizedSettings)
  const imageProfile: ApiProfile = {
    ...activeProfile,
    model: ecommerceCapabilities.defaultImageModel || activeProfile.model,
    apiMode: 'images',
    responseFormatB64Json: true,
  }
  const requestSettings = createSettingsForApiProfile(normalizedSettings, imageProfile)
  const validation = validateApiProfile(imageProfile)
  if (validation) throw new Error(`请先完善请求 API 配置：${validation}`)
  ...
  const normalizedParams = normalizeParamsForSettings(
    { ...options.params, n: 1 },
    requestSettings,
    { hasInputImages: options.inputImageIds.length > 0 },
  )
  const taskId = genId()
  const task: TaskRecord = {
    id: taskId,
    ...
    finishedAt: null,
    elapsed: null,
    sourceMode: 'ecommerce',
    suiteId: options.suiteId,
    suiteGroup: options.suiteGroup,
    suiteItemId: options.suiteItemId,
    suiteItemTitle: options.suiteItemTitle,
  }

  const latestTasks = useStore.getState().tasks
  useStore.getState().setTasks([task, ...latestTasks])
  await putTask(task)
  showToast(`已提交：${options.suiteItemTitle}`, 'success')
  await executeTask(taskId)
  return taskId
}

async function runEcommerceGeneration(onlyItemId?: string): Promise<void> {
  const state = useStore.getState()
  const suite = state.ecommerceSuites.find((item) => item.id === state.activeEcommerceSuiteId)
  if (!suite) {
    state.showToast('请先创建电商套图工程', 'error')
    return
  }
  if (!suite.assets.some((asset) => asset.kind === 'main')) {
    state.showToast('请至少上传一张主商品图', 'error')
  ...
  const runnableItems = suite.plan.filter((item) => item.enabled && (!onlyItemId || item.id === onlyItemId))
  if (runnableItems.length === 0) {
    state.showToast('没有可生成的套图计划项', 'error')
    return
  }

  const baseParams = useStore.getState().params
  const jobs = runnableItems.flatMap((item) =>
    Array.from({ length: Math.max(1, item.count) }, (_, index) => ({
      key: `${item.id}:${index + 1}:${Date.now().toString(36)}`,
      itemId: item.id,
    })),
  )
  const itemTotals = new Map<string, number>()
  const itemFailures = new Map<string, number>()
  for (const job of jobs) itemTotals.set(job.itemId, (itemTotals.get(job.itemId) ?? 0) + 1)

  useStore.setState((latest) => updateActiveEcommerceSuiteState(latest, (currentSuite) => {
    if (currentSuite.id !== suite.id) return currentSuite
    return {
      ...currentSuite,
      plan: currentSuite.plan.map((item) =>
        itemTotals.has(item.id)
          ? {
              ...item,
              status: 'generating',
              error: null,
              ...(onlyItemId ? { outputTaskIds: item.outputTaskIds } : {}),
            }
          : item,
      ),
    }
  }))
  useStore.setState({ generationQueue: jobs.map((job) => job.key) })

  let cursor = 0
  const nextJob = () => jobs[cursor++]
  const runWorker = async () => {
    while (true) {
      const job = nextJob()
      if (!job) return
      try {
        const latestSuite = useStore.getState().ecommerceSuites.find((item) => item.id === suite.id)
        const latestItem = latestSuite?.plan.find((item) => item.id === job.itemId)
        if (!latestSuite || !latestItem) throw new Error('套图计划项已不存在')
        const prompt = latestItem.prompt || latestItem.finalPromptDraft || buildEcommerceItemPrompt(latestSuite.brief, latestItem, latestSuite.assets)
        const taskId = await submitEcommerceImageTask({
          prompt,
          inputImageIds: selectEcommerceInputImageIds(latestSuite, latestItem),
          params: baseParams,
          suiteId: latestSuite.id,
          suiteGroup: latestItem.group,
          suiteItemId: latestItem.id,
          suiteItemTitle: latestItem.title,
        })
        const task = useStore.getState().tasks.find((item) => item.id === taskId)
        if (!task || task.status !== 'done' || task.outputImages.length === 0) {
          throw new Error(task?.error || '任务未返回可用图片')
        }
        useStore.setState((latest) => updateActiveEcommerceSuiteState(latest, (currentSuite) =>
          currentSuite.id === suite.id
            ? updateSuitePlanItem(currentSuite, job.itemId, {
                outputTaskIds: Array.from(new Set([
                  ...(currentSuite.plan.find((item) => item.id === job.itemId)?.outputTaskIds ?? []),
                  taskId,
                ])),
              })
            : currentSuite,
        ...
        itemFailures.set(job.itemId, (itemFailures.get(job.itemId) ?? 0) + 1)
        useStore.setState((latest) => updateActiveEcommerceSuiteState(latest, (currentSuite) =>
          currentSuite.id === suite.id
            ? updateSuitePlanItem(currentSuite, job.itemId, {
        ...
        useStore.setState((latest) => ({
          generationQueue: latest.generationQueue.filter((key) => key !== job.key),
        }))
      }
    }
  }

  await Promise.all(Array.from({ length: Math.min(2, jobs.length) }, () => runWorker()))
  ...
  useStore.setState((latest) => updateActiveEcommerceSuiteState(latest, (currentSuite) => {
    ...
    return {
      ...currentSuite,
      plan: currentSuite.plan.map((item) => {
        const total = itemTotals.get(item.id)
        if (!total) return item
        const failed = itemFailures.get(item.id) ?? 0
        if (failed === 0) return { ...item, status: 'done', error: null }
        if (item.outputTaskIds.length > 0) return { ...item, status: 'partial_success', error: `${failed}/${total} 个任务失败` }
        return { ...item, status: 'error', error: item.error || `${failed}/${total} 个任务失败` }
      }),
    }
  }))

  const latestSuite = useStore.getState().ecommerceSuites.find((item) => item.id === suite.id)
  const failedCount = latestSuite?.plan.filter((item) => itemTotals.has(item.id) && (item.status === 'error' || item.status === 'partial_success')).length ?? 0
  useStore.getState().showToast(failedCount ? `套图生成完成，${failedCount} 项存在失败` : '套图生成完成', failedCount ? 'error' : 'success')
}
...
async function executeTask(taskId: string) {
  ...
  const activeProfile = applyTaskApiProfileOverrides(taskProfile ?? getActiveApiProfile(settings), task)
