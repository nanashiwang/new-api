...
export type AppMode = 'gallery' | 'agent' | 'ecommerce'
...
export type ProductAssetKind = 'main' | 'angle' | 'sku' | 'brand' | 'style'
export type SuitePlanGroup = 'main' | 'detail' | 'promotion' | 'sku'
export type EcommercePlanStatus = 'idle' | 'planning' | 'ready' | 'error'
export type SuitePlanItemStatus = 'draft' | 'generating' | 'done' | 'partial_success' | 'error'
...
export interface EcommerceBrief {
  ...
  lockProduct: boolean
  lockBrandStyle: boolean
  allowBeautify: boolean
  strictStructure: boolean
  counts: Record<SuitePlanGroup, number>
}

export interface SuitePlanItem {
  id: string
  group: SuitePlanGroup
  title: string
  purpose: string
  ratio: string
  prompt: string
  ...
  count: number
  enabled: boolean
  status: SuitePlanItemStatus
  outputTaskIds: string[]
  error?: string | null
}

export interface EcommerceSuite {
  id: string
  name: string
