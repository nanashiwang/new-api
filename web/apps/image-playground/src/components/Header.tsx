...
export default function Header() {
  ...
  const ecommerceCapabilities = useStore((s) => s.ecommerceCapabilities)
  ...
  const handleEcommerceClick = () => {
    if (!ecommerceCapabilities.supportsEcommerce) {
      setConfirmDialog({
        title: '电商套图暂不可用',
        message: ecommerceCapabilities.disabledReason || '当前令牌分组未同时支持 gpt-image-2 与 gpt-5.5。',
        showCancel: false,
        confirmText: '我知道了',
        icon: 'info',
        action: () => {},
        ...
        <div className="safe-area-x safe-header-inner max-w-7xl mx-auto flex items-center justify-between relative">
          ...
          <div className="hidden sm:flex items-center gap-1 rounded-xl border border-gray-200 dark:border-white/[0.08] bg-gray-100/70 dark:bg-white/[0.04] p-1 mr-4">
            ...
            <button
              ...
              disabled={!ecommerceCapabilities.supportsEcommerce}
              title={ecommerceCapabilities.supportsEcommerce ? '电商套图' : ecommerceCapabilities.disabledReason || '当前配置不支持电商套图'}
              className={`px-4 py-1.5 rounded-lg text-sm transition-colors disabled:cursor-not-allowed disabled:opacity-45 ${appMode === 'ecommerce' ? 'bg-white dark:bg-white/10 text-gray-900 dark:text-white shadow-sm font-medium' : 'text-gray-500 hover:text-gray-800 dark:hover:text-gray-200'}`}
            >
              电商套图
            </button>
          </div>
