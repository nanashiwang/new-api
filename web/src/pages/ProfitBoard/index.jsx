...
const ProfitBoardPage = () => {
  ...
  const cachedBundle = useMemo(() => normalizeCachedReportBundle(safeParse(localStorage.getItem(REPORT_CACHE_KEY), null)), []);
  const restoredState = useMemo(() => normalizeRestoredState(safeParse(localStorage.getItem(STORAGE_KEY), {})), []);
  ...
  const [draft, setDraft] = useState(restoredState.draft || createDefaultDraft());
  const [editingBatchId, setEditingBatchId] = useState(restoredState.editingBatchId || '');
  const [dateRange, setDateRange] = useState(restoredState.dateRange);
  const [granularity, setGranularity] = useState(restoredState.granularity || 'day');
  const [customIntervalMinutes, setCustomIntervalMinutes] = useState(restoredState.customIntervalMinutes || 15);
  const [chartTab, setChartTab] = useState(restoredState.chartTab || 'trend');
  const [metricKey, setMetricKey] = useState(restoredState.metricKey || 'configured_profit_usd');
  const [analysisMode, setAnalysisMode] = useState(restoredState.analysisMode || 'business_compare');
  const [viewBatchId, setViewBatchId] = useState(restoredState.viewBatchId || 'all');
  const [detailFilter, setDetailFilter] = useState(restoredState.detailFilter || null);
  const [comboConfigs, setComboConfigs] = useState(restoredState.comboConfigs || []);
  const [upstreamConfig, setUpstreamConfig] = useState(restoredState.upstreamConfig || {});
  ...
  const [lastQueryKey, setLastQueryKey] = useState(cachedBundle?.queryKey || restoredState.lastQueryKey || '');
  const [autoRefreshMode, setAutoRefreshMode] = useState(restoredState.autoRefreshMode || false);
  const [hasNewActivity, setHasNewActivity] = useState(false);
  const [detailRows, setDetailRows] = useState([]);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailPage, setDetailPage] = useState(restoredState.detailPage || 1);
  const [detailPageSize, setDetailPageSize] = useState(restoredState.detailPageSize || 12);
  const [detailTotal, setDetailTotal] = useState(0);
  const lastActivityWatermarkRef = useRef(cachedBundle?.activityWatermark || '');

  const channelOptions = useMemo(() => (options.channels || []).map((item) => ({ label: item.tag ? `${item.name} (${item.tag})` : item.name, value: String(item.id) })), [options.channels]);
  const channelMap = useMemo(() => new Map((options.channels || []).map((item) => [String(item.id), item])), [options.channels]);
  ...
  const localModelMap = useMemo(() => new Map((options.local_models || []).map((item) => [item.model_name, item])), [options.local_models]);
  const modelNameOptions = useMemo(() => (options.local_models || []).map((item) => ({ label: item.model_name, value: item.model_name })), [options.local_models]);
  const batchPayload = useMemo(() => batches.map((batch) => ({ id: batch.id, name: batch.name?.trim() || t('未命名组合'), scope_type: batch.scope_type, channel_ids: (batch.channel_ids || []).map((item) => Number(item)).filter(Boolean), tags: batch.tags || [] })), [batches, t]);
  const configLookupKey = useMemo(() => JSON.stringify(batchPayload), [batchPayload]);
  ...
  const duplicateBatchError = useMemo(() => {
    ...
    for (const batch of batches) {
      const channelIds = batch.scope_type === 'tag' ? (batch.tags || []).flatMap((tag) => tagChannelMap.get(tag) || []) : batch.channel_ids || [];
  ...
  const validationErrors = useMemo(() => {
    ...
    if (comboConfigs.some((item) => item.site_mode === 'shared_site_model') && !(siteConfig.model_names || []).length) {
      errors.push(t('有组合启用了读取本站模型价格，请至少选择一个本站模型'));
    }
    if (!Array.isArray(dateRange) || !dateRange[0] || !dateRange[1]) errors.push(t('请选择完整的时间范围'));
    return errors;
  }, [batches.length, comboConfigs, dateRange, duplicateBatchError, siteConfig.model_names, t]);

  const configPayload = useMemo(() => ({
    batches: batchPayload,
    shared_site: { model_names: siteConfig.model_names || [], group: siteConfig.group || '', use_recharge_price: !!siteConfig.use_recharge_price },
  ...
  const queryPayload = useMemo(() => ({
    ...configPayload,
    start_timestamp: Math.floor(new Date(dateRange?.[0] || 0).getTime() / 1000),
    end_timestamp: Math.floor(new Date(dateRange?.[1] || 0).getTime() / 1000),
    granularity,
    custom_interval_minutes: granularity === 'custom' ? customIntervalMinutes : 0,
    include_details: false,
    detail_limit: 0,
  }), [configPayload, customIntervalMinutes, dateRange, granularity]);

  const persistState = useCallback(() => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ batches, draft, editingBatchId, dateRange, granularity, customIntervalMinutes, chartTab, metricKey, analysisMode, viewBatchId, detailFilter, comboConfigs, upstreamConfig, siteConfig, lastQueryKey, detailPage, detailPageSize, autoRefreshMode }));
  }, [analysisMode, autoRefreshMode, batches, chartTab, comboConfigs, customIntervalMinutes, dateRange, detailFilter, detailPage, detailPageSize, draft, editingBatchId, granularity, lastQueryKey, metricKey, siteConfig, upstreamConfig, viewBatchId]);
  useEffect(() => { persistState(); }, [persistState]);

  useEffect(() => {
    setComboConfigs((prev) => batches.map((batch) => {
      const existing = (prev || []).find((item) => item.combo_id === batch.id);
      const fallback = createDefaultComboPricingConfig(batch.id, siteConfig, upstreamConfig);
      return {
        ...fallback,
        ...(existing || {}),
        combo_id: batch.id,
        site_rules: (existing?.site_rules || fallback.site_rules || []).map((rule) => createDefaultPricingRule(rule)),
        upstream_rules: (existing?.upstream_rules || fallback.upstream_rules || []).map((rule) => createDefaultPricingRule(rule)),
      };
    }));
  ...
  const loadOptions = useCallback(async () => {
    ...
    if (!res.data.success) throw new Error(res.data.message || t('加载选项失败'));
    setOptions(res.data.data || { channels: [], tags: [], groups: [], local_models: [] });
  }, [t]);

  const loadConfig = useCallback(async () => {
    if (!configLookupKey || configLookupKey === '[]') return;
    const res = await API.get('/api/profit_board/config', { params: { batches: configLookupKey } });
    if (!res.data.success) throw new Error(res.data.message || t('加载配置失败'));
    const config = res.data.data?.config;
    if (!config) return;
    setSiteConfig((prev) => ({ ...prev, ...(config.shared_site || {}), model_names: config.shared_site?.model_names || [] }));
    setUpstreamConfig((prev) => ({ ...prev, ...(config.upstream || {}) }));
    setComboConfigs((config.combo_configs || []).map((item) => ({
      ...createDefaultComboPricingConfig(item.combo_id || '', config.site, config.upstream),
      ...item,
  ...
  useEffect(() => {
    const bootstrap = async () => {
      setLoading(true);
      try { await loadOptions(); } catch (error) { showError(error); } finally { setLoading(false); }
    };
    bootstrap();
  }, [loadOptions]);

  useEffect(() => { if (batchPayload.length) loadConfig().catch(showError); }, [batchPayload.length, loadConfig]);
  ...
  const runOverviewQuery = useCallback(async () => {
    ...
    } catch (error) { showError(error); } finally { setOverviewQuerying(false); }
  ...
  const runQuery = useCallback(async () => {
    ...
    try {
      ...
      lastActivityWatermarkRef.current = nextReport?.meta?.activity_watermark || '';
      localStorage.setItem(REPORT_CACHE_KEY, JSON.stringify({ report: nextReport, queryKey: currentQueryKey, activityWatermark: nextReport?.meta?.activity_watermark || '' }));
    } catch (error) { showError(error); } finally { setQuerying(false); }
  }, [currentQueryKey, queryPayload, validationErrors]);

  const runFullRefresh = useCallback(async () => { await runOverviewQuery(); await runQuery(); }, [runOverviewQuery, runQuery]);

  const checkActivity = useCallback(async () => {
    if (!autoRefreshMode || !autoRefreshEligible || validationErrors.length > 0) return;
    ...
    try {
      ...
      if (!lastActivityWatermarkRef.current) lastActivityWatermarkRef.current = nextWatermark;
      else if (nextWatermark && nextWatermark !== lastActivityWatermarkRef.current) setHasNewActivity(true);
    } catch (error) {
    } finally { setActivityChecking(false); }
  }, [autoRefreshEligible, autoRefreshMode, queryPayload, validationErrors.length]);
  ...
  const loadDetailPage = useCallback(async (page = detailPage, pageSize = detailPageSize) => {
    ...
    try {
      const res = await API.post('/api/profit_board/details', { ...queryPayload, include_details: true, detail_limit: DETAIL_LIMIT, page, page_size: pageSize, view_batch_id: viewBatchId, detail_filter: detailFilter || {} });
      if (!res.data.success) return showError(res.data.message);
      setDetailRows(res.data.data?.rows || []);
      setDetailTotal(res.data.data?.total || 0);
    } catch (error) { showError(error); } finally { setDetailLoading(false); }
  }, [detailFilter, detailPage, detailPageSize, queryPayload, reportMatchesCurrentFilters, validationErrors.length, viewBatchId]);

  useEffect(() => {
    if (!reportMatchesCurrentFilters) { setDetailRows([]); setDetailTotal(0); return; }
    loadDetailPage(detailPage, detailPageSize);
  }, [detailFilter, detailPage, detailPageSize, loadDetailPage, reportMatchesCurrentFilters, viewBatchId]);

  const addOrUpdateBatch = useCallback(() => {
    const nextBatch = { id: editingBatchId || normalizeBatchForState({}, batches.length).id, name: draft.name?.trim() || `组合 ${batches.length + (editingBatchId ? 0 : 1)}`, scope_type: draft.scope_type, channel_ids: draft.scope_type === 'channel' ? draft.channel_ids || [] : [], tags: draft.scope_type === 'tag' ? draft.tags || [] : [] };
    const selectedCount = nextBatch.scope_type === 'channel' ? nextBatch.channel_ids.length : nextBatch.tags.length;
    if (!selectedCount) return showError(t('请先选择渠道或标签'));
    setBatches((prev) => editingBatchId ? prev.map((item) => item.id === editingBatchId ? nextBatch : item) : [...prev, nextBatch]);
  ...
  const editBatch = useCallback((batch) => {
    ...
    setDraft({ id: batch.id, name: batch.name, scope_type: batch.scope_type, channel_ids: batch.channel_ids || [], tags: batch.tags || [] });
  }, []);
  const resetDraft = useCallback(() => { setDraft(createDefaultDraft()); setEditingBatchId(''); }, []);
  const removeBatch = useCallback((batchId) => {
    setBatches((prev) => prev.filter((item) => item.id !== batchId));
    setComboConfigs((prev) => prev.filter((item) => item.combo_id !== batchId));
    if (editingBatchId === batchId) resetDraft();
  }, [editingBatchId, resetDraft]);
  const updateComboConfig = useCallback((comboId, updater) => setComboConfigs((prev) => prev.map((item) => item.combo_id === comboId ? { ...item, ...(typeof updater === 'function' ? updater(item) : updater) } : item)), []);
  const addComboRule = useCallback((comboId, field, initialRule = {}) => updateComboConfig(comboId, (current) => ({ [field]: [...(current[field] || []), createDefaultPricingRule(initialRule)] })), [updateComboConfig]);
  const updateComboRule = useCallback((comboId, field, index, patch) => updateComboConfig(comboId, (current) => ({ [field]: (current[field] || []).map((item, itemIndex) => itemIndex === index ? { ...item, ...patch } : item) })), [updateComboConfig]);
  const removeComboRule = useCallback((comboId, field, index) => updateComboConfig(comboId, (current) => ({ [field]: (current[field] || []).filter((_, itemIndex) => itemIndex !== index) })), [updateComboConfig]);

  const resolveSharedSitePreview = useCallback((modelName) => {
    const model = localModelMap.get(modelName);
    if (!model) return null;
    if (siteConfig.group && (model.enable_groups || []).length > 0 && !(model.enable_groups || []).includes(siteConfig.group)) return null;
    if (model.quota_type === 1) return { input_price: clampNumber(model.model_price), output_price: 0, cache_read_price: 0, cache_creation_price: 0 };
    const factor = siteConfig.use_recharge_price ? clampNumber(model.model_price || 1) : 1;
    const baseInput = clampNumber(model.model_ratio) * 2 * factor;
    return {
      input_price: baseInput,
      output_price: clampNumber(model.model_ratio) * clampNumber(model.completion_ratio) * 2 * factor,
      cache_read_price: model.supports_cache_read ? baseInput * clampNumber(model.cache_ratio) : 0,
      cache_creation_price: model.supports_cache_creation ? baseInput * clampNumber(model.cache_creation_ratio) : 0,
    };
  }, [localModelMap, siteConfig.group, siteConfig.use_recharge_price]);
  const batchSummaryOptions = useMemo(() => [{ label: t('全部组合'), value: 'all' }, ...[...(overviewReport?.batch_summaries || []), ...(report?.batch_summaries || [])].map((item) => ({ label: item.batch_name, value: item.batch_id }))], [overviewReport?.batch_summaries, report?.batch_summaries, t]);
  const businessMetrics = useMemo(() => [{ key: 'configured_site_revenue_usd', label: t('本站配置收入') }, { key: 'upstream_cost_usd', label: t('上游费用') }, { key: 'configured_profit_usd', label: t('配置利润') }], [t]);
  ...
  const handleChartClick = useCallback((type) => (event) => {
    const label = event?.datum?.label || event?.datum?.bucket;
    if (!label) return;
    setDetailPage(1);
    setDetailFilter({ type, value: label, batchId: event?.datum?.batch_id || null });
  }, []);
  const chartContent = useMemo(() => ({
    trend: trendRows.length ? <VChart key={`trend-${actualTheme}-${analysisMode}-${viewBatchId}-${metricKey}`} spec={trendSpec} onClick={handleChartClick('trend')} /> : <Empty description={t('当前没有趋势数据')} />,
    channel: channelRows.length ? <VChart key={`channel-${actualTheme}-${analysisMode}-${viewBatchId}-${metricKey}`} spec={channelSpec} onClick={handleChartClick('channel')} /> : <Empty description={t('当前没有渠道数据')} />,
    model: modelRows.length ? <VChart key={`model-${actualTheme}-${analysisMode}-${viewBatchId}-${metricKey}`} spec={modelSpec} onClick={handleChartClick('model')} /> : <Empty description={t('当前没有模型数据')} />,
  }), [actualTheme, analysisMode, channelRows.length, channelSpec, handleChartClick, metricKey, modelRows.length, modelSpec, t, trendRows.length, trendSpec, viewBatchId]);

  const detailColumns = useMemo(() => [
    { title: t('时间'), dataIndex: 'created_at', render: (value) => timestamp2string(value) },
    { title: t('组合'), dataIndex: 'batch_name' },
    { title: t('渠道'), dataIndex: 'channel_name', render: (value, row) => value || `#${row.channel_id}` },
    { title: t('模型'), dataIndex: 'model_name' },
    { title: t('本站实际收入'), dataIndex: 'actual_site_revenue_usd', render: (value) => formatMoney(value, statusState?.status) },
    { title: t('本站配置收入'), dataIndex: 'configured_site_revenue_usd', render: (value) => formatMoney(value, statusState?.status) },
    { title: t('配置与实际差值'), dataIndex: 'configured_actual_delta_usd', render: (value) => formatMoney(value, statusState?.status) },
    { title: t('本站配置来源'), dataIndex: 'site_pricing_source', render: (value, row) => <Tag color={row.site_pricing_known ? 'blue' : 'grey'}>{sitePricingSourceLabelMap[value] || value || t('未知')}</Tag> },
    { title: t('上游费用'), dataIndex: 'upstream_cost_usd', render: (value, row) => row.upstream_cost_known ? formatMoney(value, statusState?.status) : <Text type='tertiary'>-</Text> },
    { title: t('配置利润'), dataIndex: 'configured_profit_usd', render: (value, row) => row.upstream_cost_known && row.site_pricing_known ? formatMoney(value, statusState?.status) : <Text type='tertiary'>-</Text> },
  ...
  const diagnosticSummaryCards = useMemo(() => !overviewReport?.summary ? [] : [
  ...
  const combinedWarnings = useMemo(() => Array.from(new Set([...(overviewReport?.warnings || []), ...(report?.warnings || []), ...validationErrors])), [overviewReport?.warnings, report?.warnings, validationErrors]);
  const warningSummary = combinedWarnings.length ? t('{{count}} 个需要关注的问题', { count: combinedWarnings.length }) : t('当前没有需要处理的收益口径提示');
  const detailFilterText = useMemo(() => !detailFilter?.value ? '' : `${({ trend: t('时间桶'), channel: t('渠道'), model: t('模型') }[detailFilter.type] || t('筛选'))}：${detailFilter.value}`, [detailFilter, t]);
  const batchDigest = useCallback((batch) => batch.scope_type === 'channel' ? (batch.channel_ids || []).map((id) => channelMap.get(String(id))?.name).filter(Boolean).slice(0, 3).join('、') : (batch.tags || []).slice(0, 3).join('、'), [channelMap]);
  const generatedAtText = report?.meta?.generated_at ? timestamp2string(report.meta.generated_at) : t('尚未生成');
  const sharedSiteModelCount = comboConfigs.some((item) => item.site_mode === 'shared_site_model') ? siteConfig.model_names?.length || 0 : 0;
  ...
  return (
    <Spin spinning={loading}>
      <div className='mt-[60px] space-y-4 px-2 pb-6'>
        ...
        <ChartAnalysisCard analysisMode={analysisMode} setAnalysisMode={setAnalysisMode} metricKey={metricKey} setMetricKey={setMetricKey} metricOptions={metricOptions} viewBatchId={viewBatchId} setViewBatchId={setViewBatchId} batchSummaryOptions={batchSummaryOptions} granularity={granularity} setGranularity={setGranularity} customIntervalMinutes={customIntervalMinutes} setCustomIntervalMinutes={setCustomIntervalMinutes} detailFilter={detailFilter} clearDetailFilter={() => { setDetailFilter(null); setDetailPage(1); }} runQuery={runQuery} querying={querying} chartTab={chartTab} setChartTab={setChartTab} report={report} chartContent={chartContent} t={t} />
        <DetailTableCard detailFilterText={detailFilterText} detailRows={detailRows} detailTotal={detailTotal} detailPage={detailPage} detailPageSize={detailPageSize} setDetailPage={setDetailPage} setDetailPageSize={setDetailPageSize} detailColumns={detailColumns} detailLoading={detailLoading} report={report} isMobile={isMobile} t={t} />
      </div>
    </Spin>
  );
};
